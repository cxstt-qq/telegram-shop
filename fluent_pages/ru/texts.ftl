...
buy_success = 🎉 Успешная покупка! 
    Вот твои данные:
    <code>{ $item_data }</code>
buy_success_file = 🎉 Успешная покупка!
    Данные слишком большие для одного сообщения, поэтому я отправил их файлом ниже.
buy_success_file_caption = 📄 Ваши данные покупки
